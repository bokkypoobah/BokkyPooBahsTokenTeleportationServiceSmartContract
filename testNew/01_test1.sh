...
var libSMAbi = JSON.parse(tokenOutput.contracts["$CROWDSALESOL:SafeMath"].abi);
var libSMBin = "0x" + tokenOutput.contracts["$CROWDSALESOL:SafeMath"].bin;
var libBTTSAbi = JSON.parse(tokenOutput.contracts["$CROWDSALESOL:BTTSLib"].abi);
var libBTTSBin = "0x" + tokenOutput.contracts["$CROWDSALESOL:BTTSLib"].bin;
...
// console.log("DATA: libSMAbi=" + JSON.stringify(libSMAbi));
// console.log("DATA: libSMBin=" + JSON.stringify(libSMBin));
...
// -----------------------------------------------------------------------------
var deployLibSMMessage = "Deploy SafeMath Library";
// -----------------------------------------------------------------------------
console.log("RESULT: " + deployLibSMMessage);
var libSMContract = web3.eth.contract(libSMAbi);
// console.log(JSON.stringify(libSMContract));
var libSMTx = null;
var libSMAddress = null;
var libSM = libSMContract.new({from: contractOwnerAccount, data: libSMBin, gas: 6000000, gasPrice: defaultGasPrice},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
        libSMTx = contract.transactionHash;
      } else {
        libSMAddress = contract.address;
        addAccount(libSMAddress, "SafeMath Library");
        console.log("DATA: libSMAddress=" + libSMAddress);
      }
    }
  }
);
...
printTxData("libSMTx", libSMTx);
printBalances();
failIfTxStatusError(libSMTx, deployLibSMMessage);
...
// console.log("RESULT: old='" + libBTTSBin + "'");
var newLibBTTSBin = libBTTSBin.replace(/__BTTSTokenLibrary\.sol\:SafeMath_________/g, libSMAddress.substring(2, 42));
// console.log("RESULT: new='" + newLibBTTSBin + "'");
var libBTTSContract = web3.eth.contract(libBTTSAbi);
// console.log(JSON.stringify(libBTTSContract));
var libBTTSTx = null;
var libBTTSAddress = null;
var libBTTS = libBTTSContract.new({from: contractOwnerAccount, data: newLibBTTSBin, gas: 6000000, gasPrice: defaultGasPrice},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
...
var newFactoryBin = factoryBin.replace(/__BTTSTokenLibrary\.sol\:SafeMath_________/g, libSMAddress.substring(2, 42));
newFactoryBin = newFactoryBin.replace(/__BTTSTokenLibrary\.sol\:BTTSLib__________/g, libBTTSAddress.substring(2, 42));
